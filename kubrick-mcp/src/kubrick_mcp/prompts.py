...
def routing_system_prompt() -> str:
    """Latest version of the routing prompt from Opik"""
    ...
    try:
        ...
        logger.warning("Couldn't retrieve prompt from Opik, check credential! using hardcoded prompt")
        logger.warning(f"Using hardcoded prompt: {ROUTING_SYSTEM_PROMPT}")
        prompt = ROUTING_SYSTEM_PROMPT
    return prompt

def tool_use_system_prompt() -> str:
    """Latest version of the tool use prompt from Opik"""
    ...
    try:
        ...
        if prompt is None:
            ...
            logger.info(f"Tool use prompt created. \n {prompt.commit=} \n {prompt.prompt}")
        return prompt.prompt
    except Exception:
...
def general_system_prompt() -> str:
    """Latest version of the general system prompt from Opik"""
